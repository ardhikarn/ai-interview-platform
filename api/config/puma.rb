...
# Native Windows Ruby cannot fork, so it must run Puma in single mode.
# Linux containers keep the configured cluster worker count.
workers Gem.win_platform? ? 0 : ENV.fetch('WEB_CONCURRENCY', 2)
